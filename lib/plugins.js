"use strict"

const proc = require('child_process');
const http = require('http');
const https = require('https');
const fs = require('fs');
const path = require('path');
const url = require('url');
const zlib = require('zlib');

function format_plugins(plugin) {
  return `**:: ${plugin.name}**
  ***Id:*** ${plugin.id}
  ***Repo:*** ${plugin.repo}
  ***Description:*** ${plugin.description}\n`;
}

module.exports = {};

function plugins_list(appkit, args) {
  appkit.api.get('/plugins', 
    appkit.terminal.format_objects.bind(null, format_plugins, 
      appkit.terminal.markdown('###===### No plugins were found.')));
}

function plugins_info(appkit, args) {
  console.assert(args.NAME, 'The name for the plugin wasnt provided');
  appkit.api.get('/plugins/' + args.NAME, appkit.terminal.print);
}

function plugins_publish(appkit, args) {
  console.assert(args.NAME, 'The name for the plugin wasnt provided');
  let payload = {
    description:args.description,
    name:args.NAME,
    repo:args.repo,
    owner:args.owner,
    email:args.email
  }
  let task = appkit.terminal.task(`Creating plugin **:: ${args.NAME}** `);
  task.start();
  appkit.api.post(JSON.stringify(payload), '/plugins', (err, plugin) => {
    if(err) {
      task.end('error');
      return appkit.terminal.error(err);
    }
    task.end('ok');
  });
}

function plugins_unpublish(appkit, args) {
  console.assert(args.NAME, 'The name for the plugin wasnt provided');
  let task = appkit.terminal.task(`Destroying plugin **:: ${args.NAME}** `);
  task.start();
  appkit.api.delete('/plugins/' + args.NAME, (err, plugin) => {
    if(err) {
      task.end('error');
      return appkit.terminal.error(err);
    }
    task.end('ok');
  });
}

function plugins_revise(appkit, args) {
  console.assert(args.NAME, 'The name for the plugin wasnt provided');
  let payload = {};

  if(args.description) {
    payload.description = args.description;
  }
  if(args.repo) {
    payload.repo = args.repo;
  }
  if(args.owner) {
    payload.owner = args.owner;
  }
  if(args.email) {
    payload.email = args.email;
  }
  let task = appkit.terminal.task(`Updating plugin **:: ${args.NAME}** `);
  task.start();
  appkit.api.patch(JSON.stringify(payload), '/plugins/' + args.NAME, (err, plugin) => {
    if(err) {
      task.end('error');
      return appkit.terminal.error(err);
    }
    task.end('ok');
  });
}

function plugins_install(appkit, args) {
  let pull_and_install = () => {
    let tmp_dir = null;
    let loader = appkit.terminal.task(`Installing plugin from ${args.GITHUB_REPO}`);
    loader.start();
    try {
      tmp_dir = path.join(appkit.config.third_party_plugins_dir, (".tmp" + Math.floor(Math.random() * 1000)));
      fs.mkdirSync(tmp_dir);
      proc.spawnSync('git', ['clone', args.GITHUB_REPO, tmp_dir], {cwd:process.cwd(), env:process.env, stdio:'ignore'});
      if(fs.statSync(path.join(tmp_dir, 'index.js')).isFile()) {
        let plugin = require(path.join(tmp_dir, 'index.js'));
        let plugin_name = plugin.group;
        console.assert(plugin_name.indexOf(' ') === -1, 'The plugin name or group contained invalid characters such as a space.');
        console.assert(plugin_name.length > 1, 'The plugin name was not valid, it was less than one character.');
        try {
          if(fs.statSync(path.join(tmp_dir, 'install.js')).isFile()) {
            require(path.join(tmp_dir, 'install.js'))
          }
        } catch (e) {
          // do nothing.
        }
        fs.renameSync(tmp_dir, path.join(appkit.config.third_party_plugins_dir, plugin_name));
        loader.end('ok');
        console.log(appkit.terminal.markdown(`###===### Installed plugin ^^${plugin_name}^^`));
      }
    } catch (e) {
      loader.end('error');
      if(e.message.indexOf("ENOTEMPTY") > -1) {
        appkit.terminal.error('The plugin is already installed (or a plugin with this name), try using "appkit update" instead.');
      } else if (e.message.indexOf("ENOENT") > -1) {
        appkit.terminal.error('The github repo provided did not appear to be a valid repo, check the repo URL and try again.');
      } else {
        appkit.terminal.error(`Unable to install plugin from ${args.GITHUB_REPO}: ${e.message}`);
      }
      if(tmp_dir) {
        console.assert(tmp_dir !== '/', 'Something really bad happened (tmp_dir === /).');
        console.assert(tmp_dir.indexOf(appkit.config.third_party_plugins_dir) > -1, 'Something bad happened tmp_dir was not in plugins path.');
        proc.spawnSync('rm',['-rf',tmp_dir], {cwd:process.cwd(), env:process.env, stdio:'inherit'});
      }
    }
  };
  if(args.GITHUB_REPO.startsWith("https://") || args.GITHUB_REPO.startsWith("git@")) {
    pull_and_install();
  } else {
    appkit.api.get('/plugins/' + args.GITHUB_REPO, (err, plugin) => {
      if(err) {
        return appkit.terminal.error('The specified plugin could not be found.');
      }

      args.GITHUB_REPO = plugin.repo;
      pull_and_install();
    });
  }
}

function plugins_uninstall(appkit, args) {
  console.assert(appkit.plugins[args.PLUGIN], 'The specified plugin does not exist, use "appkit version" to get a list of installed plugins.');
  try {
    if(fs.statSync(path.join(appkit.config.third_party_plugins_dir, args.PLUGIN, '.git')).isDirectory()) {
      let tmp_dir = path.join(appkit.config.third_party_plugins_dir, args.PLUGIN);
      console.assert(tmp_dir !== '/', 'Something really bad happened (tmp_dir === /).');
      console.assert(tmp_dir.indexOf(appkit.config.third_party_plugins_dir) > -1, 'Something bad happened tmp_dir was not in plugins path.');
      proc.spawnSync('rm',['-rf',tmp_dir], {cwd:process.cwd(), env:process.env, stdio:'inherit'});
    } else {
      throw new Error("No such plug-in found.");
    }
  } catch (e) {
    if(e.message.indexOf("ENOENT") > -1) {
      appkit.terminal.error(`You cannot uninstall the core plugin ${args.PLUGIN}, sorry.`);
    } else {
      appkit.terminal.error(`Unable to install plugin ${args.PLUGIN}: ${e.message}`);
    }
  }
}

module.exports.init = function(args, appkit) {
  let plugins_create = {
    'description':{
      'alias':'d',
      'type':'string',
      'description':'The description for the plugin.'
    },
    'repo':{
      'alias':'r',
      'type':'string',
      'demand':true,
      'description':'The repo (e.g., https://github.com/foo/bar) for the plugin.'
    },
    'owner':{
      'alias':'o',
      'type':'string',
      'demand':true,
      'description':'The owners name for the plugin.'
    },
    'email':{
      'alias':'e',
      'type':'string',
      'demand':true,
      'description':'The owners email for the plugin.'
    }
  };

  let plugins_update = {
    'description':{
      'alias':'d',
      'type':'string',
      'description':'The description for the plugin.'
    },
    'repo':{
      'alias':'r',
      'type':'string',
      'description':'The repo (e.g., https://github.com/foo/bar) for the plugin.'
    },
    'owner':{
      'alias':'o',
      'type':'string',
      'description':'The owners name for the plugin.'
    },
    'email':{
      'alias':'e',
      'type':'string',
      'description':'The owners email for the plugin.'
    }
  };

  args.command('plugins', 'list public plugins', {}, plugins_list.bind(null, appkit))
    .command('plugins:info NAME', 'get more information on a plugin', {}, plugins_info.bind(null, appkit))
    .command('plugins:publish NAME', 'publish a plugin', plugins_create, plugins_publish.bind(null, appkit))
    .command('plugins:unpublish NAME', 'unpublish a plugin', {}, plugins_unpublish.bind(null, appkit))
    .command('plugins:revise NAME', 'update a published plugin', plugins_update, plugins_revise.bind(null, appkit))
    .command('plugins:install GITHUB_REPO', 'install a plugin from a repo.', {}, plugins_install.bind(null, appkit))
    .command('plugins:uninstall PLUGIN', 'uninstall a plugin.', {}, plugins_uninstall.bind(null, appkit))   
}